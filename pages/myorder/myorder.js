...
Page({
  ...
  data: {
    ...
    order_status: 0,
    isOpenset: false,
    tableCodeAlertShow: false,//桌码弹窗是否显示开关
  },
    ...
    wx.request({
      ...
      success(res) {
        ...
        if (res.data.status == 1) {
          ...
          console.log('wzh', mydata.effective_date)
      ...
      success(res) {
        ...
        if (res.data.status == 1) {
          ...
          if (newData.hasOwnProperty('list_order') && newData.list_order.length > 0) {
            let list_order = newData.list_order;//订单列表数组
            if (arr.page == 1) {
              orderData = [];
            }
            ...
            for (let i = 0; i < list_order.length; i++) {
              ...
              if (list_order[i].order_status == 1) {
                let over_time = new Date(list_order[i].overdue_time.replace(/\.|\-/g, '/'));
                let nowDate = new Date(list_order[i].now_time.replace(/\.|\-/g, '/'));
                list_order[i].remainTime = over_time - nowDate;
              }
              if (list_order[i].category_id == 1) {//是餐饮行业
                list_order[i].categoryIs1 = true;
              } else {
                list_order[i].categoryIs1 = false;
              }
  ...
  countdown(orderData, unpaid_count) {
    // console.log('orderData', orderData)
    // console.log('unpaid_count', unpaid_count)
    let that = this;
    // clearInterval(timer);
    if (unpaid_count > 0) {
  ...
  scanTableCode() {
    return new Promise((resolve, reject) => {
      wx.scanCode({
        onlyFromCamera: true,
        success(res) {
          const result = res.result.match(/q=\d+/)
          resolve(result[0])
        }
      })
    })
  ...
  confirmCode(data) {
    let gram = util.returnGram()
    let { order_id, storeid } = data.currentTarget.dataset
    let that = this

    wx.scanCode({//扫码 获取get请求的请求链接
      onlyFromCamera: true,
      success(res) {
        wx.request({//扫码成功 拿到请求链接去发请求去获取桌号
          url: res.result,
          method: 'GET',
          success(res) {//获取桌号成功 改变状态机去显示桌码弹窗子组件
            if (res.data.status == 1) {
              that.setData({
                table_num: res.data.data.table_number,
                tableCodeAlertShow: true,
                order_id,
                store_id: storeid
              })
            }
    ...
    //   data:{
    //     order_id,
    //     user_id: wx.getStorageSync('globalKey')['user_id'],
    //     timestamp: gram.timestamp,
    //     process: gram.process,
    //     token: gram.token,
    //   },
  ...
  closeTableCodeAlert(data) {
    this.setData({
      tableCodeAlertShow: false,
    })
    let arr = this.returnArr();
    this.getData(arr[0])
    this.onLoad()
