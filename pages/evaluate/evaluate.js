...
Page({
  ...
  sentData(e){
    ...
    if (this.selectOne()){
      ...
      if (this.data.textareaValue == '') {
        ...
        if(this.data.textareaValue.length < 3){
          wx.showToast({
            title: '评论字数不能小于三个字',
            icon:'none',
          })
    ...
    console.log(temp1, '------------',temp,9801);//return
  ...
  //判断是否至少选中了一个TAG
  selectOne:function(){
    for(var item of this.data.tags){
